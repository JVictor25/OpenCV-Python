...
def plotAddedImages():
    circleImage = cv2.imread("imgs/bin1.png")
    rectangleImage = cv2.imread("imgs/bin2.png")
    addedImage = cv2.add(circleImage, rectangleImage)
    addedWeightedImage = cv2.addWeighted(circleImage, 0.2 , rectangleImage, 1, 0)
    ...
    for y in range(0, altura):
        for x in range(0, largura):
            if mask.item(y, x) == 255:
                blue = firstImage.item(y, x, 0) * (1.0 - weight) + secondImage.item(y, x, 0) * weight
                green = firstImage.item(y, x, 1) * (1.0 - weight) + secondImage.item(y, x, 1) * weight
                red = firstImage.item(y, x, 2) * (1.0 - weight) + secondImage.item(y, x, 2) * weight

                copyImg[y, x]=(blue, green, red)
...
def memeGeneratorWithBlending(fala1, imagem1, fala2, imagem2, fundo):
    ...
    cv2.imwrite("imgs/memeatila.png", finalImage)

memeGeneratorWithBlending('Respeito seu argumento!', "imgs/atila_feliz.png", 'Burro pra caramba...', "imgs/atila_bravo.png", "imgs/fundo2.jpg")

def main():
    plotAddedImages()
    atila_bravo = cv2.imread("imgs/memeatila.png")
